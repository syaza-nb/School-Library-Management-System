        ...
        <div class="card shadow-sm rounded">
            ...
            <div class="card-body table-responsive">
                <table class="table table-hover table-bordered">
                    ...
                    <tbody>
                    <?php
                    $res = mysqli_query($link, "SELECT * FROM helpdesk_tickets WHERE username = '{$_SESSION['student']}' ORDER BY id DESC");
                    if (mysqli_num_rows($res) > 0) {
                        while ($row = mysqli_fetch_assoc($res)) {
                            $statusBadge = $row['status'] === 'Resolved' 
                                ? "<span class='badge badge-success'>Resolved</span>" 
                                : "<span class='badge badge-warning'>Pending</span>";
                            $response = !empty($row['response']) ? htmlentities($row['response']) : "<em class='text-muted'>Awaiting response</em>";
                            echo "<tr>
